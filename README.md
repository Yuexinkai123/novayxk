...
- 模型可通过 `fileops` 代码块请求创建目录、写入文件或删除项目内路径；创建/写入会自动尝试执行，删除默认要求人工确认
...
为了避免把普通说明文字里的示例命令误当成真实操作，Novayxk 现在只会自动执行单独放在 `powershell-run` 代码块中的命令；普通文本里的命令示例、备选命令或解释说明不会自动运行。
...
请把这段放在 `fileops` 代码块中。模型返回后，Novayxk 会自动尝试在当前项目内执行创建目录和写入文件；如果包含 `delete`，为了避免 AI 误删项目内容，Novayxk 不会自动执行，用户需要点底部工具栏的“执行文件操作”按钮手动确认。路径必须是当前项目内的相对路径，敏感文件名会被拦截；写入已存在文件时需要显式设置 `overwrite: true`。
