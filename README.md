...
这是 Novayxk 内部的 AI 控制模式，不等于设置里的“高级权限 / 管理员模式”。用户要求切换管理员模式时，Novayxk 会请求 Windows UAC 并以管理员权限重启；遇到注册表、系统目录、安装卸载等系统级操作时，也会单独触发 Windows UAC 授权。
...
Novayxk 会在当前项目根目录运行该命令，并把输出写回聊天，同时在底部“终端任务”面板实时显示。AI 自动执行、确认弹窗执行和底部终端输入框启动的命令都会进入同一个终端任务列表。如果命令卡在 `Y/N`、协议确认或其他交互提示，Novayxk 会把任务标记为“等待输入”，你可以在终端任务下方的输入栏发送一行输入给当前运行任务。用户发送输入后，终端输出会记录“用户已插手”，方便后续判断命令结果。完全控制模式风险很高，只建议在你明确信任当前任务时开启。
