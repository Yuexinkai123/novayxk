...
Novayxk 会在当前项目根目录运行该命令，并把输出写回聊天和命令输出面板。完全控制模式风险很高，只建议在你明确信任当前任务时开启。
...
请把这段放在 `fileops` 代码块中。模型返回后，Novayxk 会自动尝试在当前项目内执行这些文件操作；如果自动执行失败，仍会在底部“变更预览”里保留操作内容，方便你手动确认后再次执行。路径必须是当前项目内的相对路径，敏感文件名会被拦截；写入已存在文件时需要显式设置 `overwrite: true`。
