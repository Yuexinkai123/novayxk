...
function App() {
  ...
  React.useEffect(() => {
    ...
    return window.novayxk.onTerminalTaskUpdate((payload) => {
      ...
      if (payload.task.status === "running" && payload.task.needsInput) {
        ...
        setStatus(`终端任务等待输入：${payload.task.title}`);
      }
    });
  }, []);
  ...
  const restartAsAdmin = async () => {
    ...
    try {
      ...
      setStatus(error instanceof Error ? error.message : "Windows 管理员启动失败");
      return false;
    }
  };
  ...
  const startTerminalTask = async () => {
    ...
    try {
      ...
      if (!adminReady) {
        setStatus("已请求 Windows 管理员授权，Novayxk 将以 Windows 管理员权限重启。");
        return;
      }
  ...
  const sendTerminalInput = async () => {
    ...
    try {
      ...
      setStatus(error instanceof Error ? error.message : "发送终端输入失败");
    }
  };

  const handleInternalControlModeRequest = async (userPrompt: string, nextMessages: ChatMessage[]) => {
    ...
    const assistantContent =
      nextMode === "full"
        ? "已切换到 Novayxk 内部的完全控制模式。这个是软件内部的 AI PowerShell 控制模式，不等于 Windows 管理员权限；如果后面遇到系统级命令，我还会单独请求 UAC/管理员授权。"
        : "已切换回 Novayxk 内部的默认权限模式。之后我只会自动请求执行低风险 PowerShell 命令，敏感系统动作会继续拦截或要求确认。";
    ...
    setStatus(saved ? (nextMode === "full" ? "已切换到完全控制模式" : "已切换到默认权限模式") : "权限模式已切换，但保存偏好失败");
    await saveCurrentTask(handledMessages);
    return true;
  };

  const handleAdminPrivilegeRequest = async (userPrompt: string, nextMessages: ChatMessage[]) => {
    ...
    if (!window.novayxk) {
      assistantContent = "当前是浏览器预览模式，不能请求 Windows 管理员权限。请用 Novayxk 桌面应用启动后再切换管理员模式。";
      setStatus("浏览器预览模式不能请求管理员权限");
    } else if (privilege?.isAdmin) {
      assistantContent = "当前已经是管理员权限模式，不需要再次切换。";
      setStatus("当前已经是管理员权限模式");
    } else if (!privilege?.canElevate) {
      assistantContent = "当前环境不能直接请求 Windows UAC 管理员权限。请确认是在打包后的 Novayxk 桌面应用里运行。";
      setStatus("当前环境不能请求管理员权限");
    } else {
      assistantContent = "我正在请求切换到管理员权限模式。接下来会弹出 Windows UAC 授权窗口；确认后 Novayxk 会以管理员权限重启。";
      setStatus("正在请求 Windows 管理员权限...");
      ...
      assistantContent = "管理员权限请求没有成功启动。可能是当前处于开发模式，或 Windows UAC 启动被取消。";
    }
  ...
  const stopGeneration = async () => {
    ...
    try {
      ...
      if (terminalResult.status === "fulfilled") {
        ...
        if (stoppedTask) {
          setTerminalTasks((current) => upsertTerminalTask(current, stoppedTask));
        }
      }
      ...
      <header className="topbar">
        ...
        <div className="top-actions">
          <div className={`privilege-chip ${privilege?.isAdmin ? "admin" : "standard"}`}>
            ...
            {privilege?.isAdmin ? "Windows 管理员" : "Windows 普通"}
          </div>
        ...
        <div className="modal-backdrop" role="presentation">
          <section className="settings-modal" role="dialog" aria-modal="true" aria-label="模型供应商设置">
            ...
            <div className={`privilege-panel ${privilege?.isAdmin ? "admin" : ""}`}>
              <div>
                <span>高级权限</span>
                <strong>{privilege?.isAdmin ? "当前是管理员权限模式" : "当前是普通权限模式"}</strong>
                <p>
                  {privilege?.isAdmin
                    ? "系统级命令、注册表和受保护目录操作会拥有更高权限。"
                    : "需要改系统设置、注册表或受保护目录时，可以通过 UAC 以 Windows 管理员权限重启。"}
                </p>
              </div>
              <button
                className="ghost-button"
                ...
                title={privilege?.isDev ? "开发模式下请打包后测试 Windows 管理员重启" : "通过 Windows UAC 以管理员权限重启 Novayxk"}
              >
                <ShieldCheck size={16} />
            ...
            {confirmDialog.type === "command" ? (
              ...
              <>
                <p className="confirm-copy">
                  ...
                  可能需要 Windows 管理员权限：{confirmDialog.reason}。确认后 Novayxk 会触发 Windows UAC 并以 Windows 管理员权限重启；当前命令不会在重启前自动执行。
                </p>
                <pre className="confirm-preview">{confirmDialog.command}</pre>
              </>
            ) : (
...
function buildSystemPrompt(projectMemory: string, taskSummary: string, runtimePermission: RuntimePermissionContext) {
  ...
  const windowsPrivilegeBlock = runtimePermission.isAdmin
    ...
    : `当前 Novayxk Windows 进程权限：${runtimePermission.privilegeLabel}。用户要求切换“管理员模式”“高级权限”“管理员权限”时，通常指设置里的高级权限按钮：Novayxk 可以请求 Windows UAC 并以管理员权限重启；不要回答成只能让用户手动右键打开。不要把 Windows 进程权限和 Novayxk 内部完全控制模式混为一谈。`;
  const controlModeBlock =
    runtimePermission.controlMode === "full"
      ? "当前 Novayxk 内部 AI PowerShell 控制模式：完全控制。用户允许你通过 ```powershell-run 代码块请求执行包括高风险命令在内的 PowerShell 命令。仍要优先解释目的，避免无意义破坏。"
      : "当前 Novayxk 内部 AI PowerShell 控制模式：默认权限。用户要求切换“完全控制”“AI 控制模式”“内部控制模式”时，才理解为内部 PowerShell 控制模式；用户只说“管理员模式”时优先理解为 Windows UAC 管理员权限请求。你可以通过 ```powershell-run 代码块请求执行低风险 PowerShell 命令，例如 npm run build、npm test、dir、Get-ChildItem、git status。不要把删除、重置、格式化、系统设置、下载后直接执行脚本等高风险命令放进自动执行块。";
  const logBlock =
  ...
  return `你是 Novayxk，一款谨慎但自然的 AI 编程助手。回答要具体、可执行，也要像正常对话一样根据用户当下的话轻重回应。用户只是打招呼、寒暄、测试在线状态或说很短的闲聊内容时，只简短自然回应，不要主动介绍项目、不要复述文件清单、不要列功能菜单。只有用户明确要求分析项目、查看文件、修改代码、解释项目内容或询问项目情况时，才使用文件清单和相关文件上下文。用户只是询问版本、环境、终端命令或运行状态时，不要介绍项目文件；如果需要命令，直接请求执行命令即可。隐藏上下文只用于帮助你判断，不要主动复述项目路径、文件清单或“我看到你的项目”。用户要求你创建页面、组件、脚本、样式或其他新文件时，优先直接返回一个 \`\`\`fileops JSON 代码块，Novayxk 会自动执行项目内文件操作。fileops 格式为 [{"type":"mkdir","path":"相对目录"},{"type":"write","path":"相对文件","content":"文件内容","overwrite":false},{"type":"delete","path":"相对路径"}]。当用户明确要求新建、覆盖整个文件、删除文件或删除目录时，优先用 fileops，不要只给说明文字。修改已有文件的小范围局部内容时，优先给出文件路径、修改理由和 diff 风格补丁；只有用户明确要求覆盖已有文件时，fileops write 才可以设置 overwrite:true。需要运行 PowerShell 命令时，必须返回一个完整的、单独成块的 \`\`\`powershell-run 代码块，每个代码块只放一条或一组相关命令；命令会在当前项目根目录执行。不要把要执行的命令放在普通文字、行内代码或普通 \`\`\`text 代码块里。你输出 powershell-run 后，Novayxk 会自动执行并把结果再交给你总结，所以不要要求用户手动执行命令、复制输出或“执行后发我”，也不要在收到执行结果前说“已开始安装”“等安装结果返回”。所有路径必须是当前项目内的相对路径；读取 Novayxk 自身日志除外。不要要求用户泄露密钥。${shellBlock}${memoryBlock}${taskBlock}`;
}
...
type ProjectContextMode = "none" | "runtime" | "full";

function detectAdminPrivilegeRequest(prompt: string) {
  ...
  if (/(?:完全控制|ai控制模式|内部控制模式|powershell控制模式|默认权限|安全模式|safe|full)/i.test(normalized)) {
    return false;
  }
  return /(?:管理员模式|管理员权限|高级权限|windows管理员|uac|以管理员身份运行|提权)/i.test(normalized);
}

function detectInternalControlModeRequest(prompt: string): AiControlMode | null {
  const normalized = prompt.trim().replace(/\s+/g, "").toLowerCase();
  ...
  if (!/(?:软件内部|内部|novayxk|ai|控制模式|权限模式|默认权限|完全控制|高风险|危险命令)/i.test(normalized)) {
    return null;
  }
  if (/(?:默认权限|普通权限|安全模式|普通模式|safe|关闭完全控制|关掉完全控制|退出完全控制|恢复默认)/i.test(normalized)) {
    return "safe";
  }
  if (/(?:完全控制|full|高风险|危险命令)/i.test(normalized)) {
    return "full";
  }
  return null;
}

function getProjectContextMode(prompt: string): ProjectContextMode {
...
function looksLikeProjectContextReply(content: string) {
