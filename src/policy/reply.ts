...
const KNOWLEDGE_STALL_PATTERN =
  /^(?:好|好的|行|嗯|那我|我)(?:，|,|\s)?(?:我(?:需要|得|要)?先|让我先|先让我|需要先).{0,24}(?:查|搜|搜索|确认|核实|看一下|查一下|搜一下|搜索一下)|^(?:我不确定|还不确定).{0,24}(?:访华|发布|属实|真假|有没有|是否)|^(?:let me|i need to|i should first).{0,32}(?:check|search|verify|confirm|look into)|^(?:i'?m not sure).{0,24}(?:whether|if)/i;
