...
const LEAD_IN_PATTERN =
  /(?:[:：]|\.\.\.|……)$|(?:帮你|我来|我先)(?:查|看|确认|检查|搜|处理|执行)(?:一下)?(?:电脑里|电脑上|当前)?(?:有没有|是否)?(?:安装|存在)?[\s\S]*$|(?:我先|先|我来|帮你).{0,12}(?:快速)?(?:扫一眼|扫一下|浏览一下|读一下|看一下|看一遍|过一遍|检查一下).{0,30}(?:再|然后|待会|稍后).{0,16}(?:总结|梳理|给你总结|告诉你)|(?:让我|我来|我先|帮你).{0,16}(?:轨迹|记录|流程|操作|api|接口|请求).{0,32}(?:读出来|梳理|整理|分析|看一下|查一下|追溯)|(?:计划如下|我的计划是|先按这个计划|分几步来|按这几步来)[\s\S]*$|(?:let me|i(?:'ll| will)|i(?:'m| am) going to|first,?\s*i(?:'ll| will)|i(?:'ll| will) first).{0,48}(?:check|look|inspect|review|scan|read|trace|analy[sz]e|verify|search|handle|run|summari[sz]e)\b[\s\S]*$|(?:here(?:'s| is) the plan|my plan is|let'?s break it down|plan:)[\s\S]*$/i;
