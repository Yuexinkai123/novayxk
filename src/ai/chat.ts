...
When the task involves online search outside the current embedded browser page, use a standalone \`\`\`web-search JSON\`\`\` block. The format may include {"query":"latest policy announcement official source","maxResults":5,"includePageContent":true,"includePageContentCount":2}. Output strict JSON only, with no comments and no explanatory prose inside the code block. Prefer one well-targeted search at a time. Add domains only when official confirmation matters and the likely official domain is clear. Use includePageContent when the answer depends on what the source page actually says rather than only the result title or snippet.
...
export function getProjectContextMode(prompt: string): ProjectContextMode {
  ...
  if (/(?:分析|梳理|查看|读取|打开|修改|改|创建|新建|删除|重构|优化|解释|排查|修复|生成|写|覆盖|美化|样式|页面|组件|代码|文件|目录|项目结构|项目情况|html|css|jsx?|tsx?|vue|json|md|toml|login|register|website|landing|dashboard)/i.test(normalized)) {
    return "full";
  }
...
export function looksLikeProjectContextReply(content: string) {
  return /项目上下文摘要|文件清单|我看到你的项目|你的项目目录|目前有\s*\d+\s*个|Project context summary|File list|I can see your project|Current browser workspace context|Browser API evidence pack|(?:^|[\s/\\])(?:login|register|index|dashboard|landing)\.html\b/i.test(content);
}
